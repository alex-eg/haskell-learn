...
toDigits n
  | n > 0 = let acc 0 xs = xs
                acc n xs = acc (n `div` 10) (n `mod` 10 : xs)
            in acc n []
  | n < 0 = undefined
...
doubleSecond xs = map (\(x, y) -> if y `mod` 2 == 0 then x * 2 else x) $
                  zip xs [1..length xs]
...
sumDigits xs = let f x = if x `div` 10 == 0 then [x] else
                           [x `div` 10, x `mod` 10]
               in sum $ concatMap f xs
